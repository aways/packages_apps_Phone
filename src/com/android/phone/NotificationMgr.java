...
public class NotificationMgr implements CallerInfoAsyncQuery.OnQueryCompleteListener{
    ...
    /* package */ void notifyMissedCall(
        ...
        if (mMissedCalls.size() == 1) {
            ...
            RelativeSizeSpan timeSpan = new RelativeSizeSpan(0.7f);
            ...
            for (MissedCallInfo info : mMissedCalls) {
                ...
                lineBuilder.append(info.name);
                lineBuilder.append("  ");

                int timeIndex = lineBuilder.length();
                lineBuilder.append(DateUtils.formatDateTime(mContext, info.date, flags));
                lineBuilder.setSpan(timeSpan, timeIndex, lineBuilder.length(), 0);
                style.addLine(lineBuilder);
